...
from tfclip.pretrain import get_pretrained_cfg
from tfclip.pretrain import get_pretrained_url
from tfclip.pretrain import list_pretrained_tags_by_model
from tfclip.tokenizer import DEFAULT_CONTEXT_LENGTH
...
from tfclip.transform import PreprocessCfg
from tfclip.transform import image_transform
from tfclip.transform import merge_preprocess_dict
...
def get_tokenizer(model_name="", context_length=None, **kwargs):
    ...
    if "sp_tokenizer_name" in text_config:
        tokenizer = SentencePieceTokenizer(
            text_config["sp_tokenizer_name"], **tokenizer_kwargs
        )
    elif "bert_tokenizer_name" in text_config:
        tokenizer = BertHubTokenizer(
            text_config["bert_tokenizer_name"], **tokenizer_kwargs
        )
    else:
...
def create_model_and_transforms(
    model_name, pretrained=None, weights_path=None, **model_kwargs
):
    ...
    model = CLIP(
        **model_cfg,
        img_mean=preprocess_cfg["mean"],
        img_std=preprocess_cfg["std"],
        bias_init=bias_init,
    ...
    if pretrained is not None:
        if weights_path is None:
            ...
            if weights_url is None:
                raise ValueError(
                    f'Pretrained weighs "{pretrained}" for model '
                    f'"{model_name}" are not ready and can\'t be '
                    f"downloaded. You can convert them locally with "
                    f"`convert_weights.py` script (open_clip should be "
                    f"installed) and supply with `weights_path` argument."
                )

            weights_hash = (
                weights_url.split("__")[-1]
                .replace(".weights.h5", "")
                .replace(".h5", "")
            )
    ...
    image_prep = image_transform(
        ...
        preprocess_cfg["interpolation"],
        preprocess_cfg["resize_mode"],
    )
