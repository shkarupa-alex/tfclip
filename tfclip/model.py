...
class CLIPTextCfg:
    ...
    bert_tokenizer_name: Optional[str] = None
...
def _build_vision_tower(embed_dim, vision_cfg, quick_gelu, img_mean, img_std):
    ...
    model = VisionTransformer(
        embed_dim, vision_cfg, quick_gelu, img_mean, img_std
    )
...
def CLIP(
    ...
    img_std,
    quick_gelu=False,
    scale_init=-np.log(0.07),
    bias_init=None,
    custom_text=False,
):
    vision = _build_vision_tower(
        embed_dim, vision_cfg, quick_gelu, img_mean, img_std
    )
    text = _build_text_tower(
        embed_dim, text_cfg, quick_gelu, custom=custom_text
    )

    s = ImageTextSimilarity(scale_init, bias_init, name="head_sim")(
        [vision.outputs[0], text.outputs[0]]
    )
    s = layers.Activation("softmax", name="head_prob", dtype="float32")(s)

    model = models.Model(
        inputs=[vision.inputs[0], text.inputs[0]], outputs=s, name="clip"
    )
