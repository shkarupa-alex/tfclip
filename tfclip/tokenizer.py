...
from keras_hub.src.utils.tensor_utils import no_convert_scope

tf_text = LazyModule("tensorflow_text")
...
class SimpleTokenizer:
    def __init__(self, context_length=DEFAULT_CONTEXT_LENGTH):
        ...
        self.max_len = max(
            [
                len(m.replace(" ", "").replace("</w>", "").encode("utf-8"))
                for m in merges
            ]
        )
        ...
        self.token2id = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(
                vocabulary, range(len(vocabulary))
            ),
            -1,
        )
        ...
        self.merge2rank = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(merges, range(len(merges))),
            self.merge_size + 1,
        )

        # Word -> id cache
        # `tf.lookup.experimental.MutableHashTable` does not support string
        # to string mapping. So we first convert to string to an integer key,
        # and use the integer key to find the value.
        self.cache_factors = tf.pow(
            tf.constant(256, dtype="int64"), tf.range(0, 8, dtype="int64")
        )[:, None]
        self.cache_id2value = tf.lookup.experimental.MutableHashTable(
            "int64", "string", ""
        )
    ...
    def __call__(self, inputs):
        with tf.device("cpu"):
            ...
            if 1 != inputs.shape.rank:
                raise ValueError(
                    f"Expecting a vector of strings. Got shape {inputs.shape}"
                )
            ...
            cache_miss = tf.equal(cache_lookup, "") & tf.not_equal(
                flat_tokens, ""
            )
            ...
            tokens = tf.RaggedTensor.from_row_splits(
                tokens.flat_values, tf.gather(tokens.row_splits, row_splits)
            )
    ...
    def _cache_key(self, keys):
        # `tf.fingerprint` converts token to an array of uint8 of length 8,
        # we need to convert it to int64.
        return tf.squeeze(
            tf.matmul(
    ...
    def _bpe_encode(self, words):
        ...
        chars = chars[
            :, : self.max_len * 2
        ]  # double max length due end-of-word marker changes merge priority
    ...
    def _bpe_merge(self, words, merge_mask):
        ...
        found_indices = tf.boolean_mask(
            tf.range(tf.size(merge_mask)), merge_mask
        )
        merge_mask = tf.tensor_scatter_nd_update(
            merge_mask, found_indices[:, None], curr_found
        )

        return tf.cond(
            tf.reduce_any(merge_mask),
            lambda: self._bpe_replace(
                words, merge_mask, curr_ranks, min_ranks, curr_found
            ),
            lambda: (words, merge_mask),
        )

    def _bpe_replace(
        self, words, merge_mask, pair_ranks, min_ranks, found_mask
    ):
        ...
        seg_shift = words.with_flat_values(
            tf.dynamic_stitch(
                [curr_idx.flat_values, skip_idx.flat_values],
                [
                    tf.cast(merge_right.flat_values, words_size.dtype),
                    tf.zeros_like(skip_idx.flat_values),
                ],
            )
        )
        ...
        merged_words = tf.strings.unsorted_segment_join(
            words.flat_values, segment_ids, num_segments, ""
        )

        # Merged sub-words
        words_lengths = words.row_lengths()
        merged_lengths = words_lengths - tf.cast(
            tf.reduce_sum(seg_shift, axis=-1), words_lengths.dtype
        )
...
class SentencePieceTokenizer:
    def __init__(
        self, tokenizer_name, context_length=DEFAULT_CONTEXT_LENGTH, clean=None
    ):
        ...
        model_path = get_file(
            origin=model_url, file_hash=model_hash, cache_subdir="tfclip"
        )
...
class BertHubTokenizer:
    def __init__(
        ...
        self.tokenizer = BertTokenizer.from_preset(
            tokenizer_name, strip_accents=strip_accents
        )
        self.context_length = context_length
    ...
    def __call__(self, inputs):
        with tf.device("cpu"):
            ...
            with no_convert_scope():
                tokens = self.tokenizer(inputs).to_tensor(0)
                tokens = tf.pad(tokens, [[0, 0], [1, 0]], constant_values=101)

            if self.strip_sep_token:
                tokens = tf.where(
                    tf.equal(tokens, int(self.strip_sep_token)), 0, tokens
                )
            ...
            if self.context_length:
                ...
                length = tf.maximum(
                    1, self.context_length - tf.shape(tokens)[1]
                )
                tokens = tf.pad(tokens, [[0, 0], [0, length]])
